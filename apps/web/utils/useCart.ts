...
// Assinatura estável de uma linha (para juntar iguais e distinguir variantes).
export function lineSignature(line: Pick<CartLine, 'menuItemId' | 'variantId' | 'addonIds'>): string {
  const addons = (line.addonIds ?? []).slice().sort().join(',');
  return `${line.menuItemId}|${line.variantId ?? ''}|${addons}`;
}

// Linha "simples" de um item (sem variante nem adicionais) — usada pelos cards.
