...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const Sidebar = (
    <aside className="flex h-full w-64 flex-col bg-white/[0.03] backdrop-blur-[12px] border-r border-white/[0.08]">
      ...
      <div className="flex items-center gap-2.5 px-5 h-16 border-b border-white/[0.08]">
        <span className="grid place-items-center w-8 h-8 rounded-lg bg-[#EA1D2C] text-white font-black shadow-[0_0_16px_rgba(234,29,44,0.4)]">D</span>
          ...
          return (
            <Link
              ...
              className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all ${
                active
                  ? 'bg-[#EA1D2C]/[0.12] text-[#EA1D2C] border-l-[3px] border-[#EA1D2C] shadow-[inset_0_0_16px_rgba(234,29,44,0.08)]'
                  : 'border-l-[3px] border-transparent text-[#A8A8B0] hover:bg-white/[0.06] hover:text-white'
              }`}
            >
              <Icon name={item.icon} />
              <span className="flex-1">{item.label}</span>
              {item.badge && badge != null && badge > 0 && (
                <span className="min-w-[20px] h-5 px-1.5 grid place-items-center rounded-full text-[11px] font-bold bg-[#EA1D2C] text-white">
                  {badge}
                </span>
              )}
            </Link>
      ...
      <div className="border-t border-white/[0.08] p-4 space-y-3">
        <div>
          <p className="text-[11px] uppercase tracking-wide text-[#6F6F78] mb-1">Status da loja</p>
        ...
        <a href="/menu" target="_blank" rel="noopener noreferrer"
          className="block text-center text-sm font-semibold rounded-xl border border-white/[0.08] py-2 text-[#e8e8ea] hover:bg-white/[0.06] transition-all">
          Ver loja
        </a>
      </div>
    </aside>
  ...
  return (
    <div className="relative min-h-screen bg-[#080809] text-[#e8e8ea] flex">
      {/* brilho ambiente para o glassmorphism ter profundidade */}
      <div aria-hidden className="pointer-events-none fixed inset-0 z-0
        bg-[radial-gradient(60rem_40rem_at_85%_-10%,rgba(234,29,44,0.10),transparent_60%),radial-gradient(50rem_40rem_at_-10%_110%,rgba(234,29,44,0.06),transparent_55%)]" />
      ...
      <div className="relative z-10 flex-1 lg:ml-64 min-w-0">
        ...
        <header className="sticky top-0 z-20 h-16 bg-white/[0.03] backdrop-blur-[12px] border-b border-white/[0.08] flex items-center gap-3 px-4 lg:px-6">
          <button onClick={() => setDrawerOpen(true)} className="lg:hidden text-[#A8A8B0] hover:text-white" aria-label="Menu">
          ...
          <div className="flex-1 max-w-md hidden sm:block">
            <div className="flex items-center gap-2 bg-black/20 border border-white/[0.08] rounded-xl px-3 py-2 backdrop-blur-[8px]">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9a9aa3" strokeWidth="2"><circle cx="11" cy="11" r="7" /><path d="m21 21-4-4" /></svg>
              <input placeholder="Buscar pedido, cliente…" className="bg-transparent text-sm text-white placeholder-[#6F6F78] focus:outline-none w-full" />
              <kbd className="hidden md:inline-flex items-center gap-0.5 rounded-md border border-white/[0.10] bg-white/[0.04] px-1.5 text-[10px] text-[#6F6F78]">⌘K</kbd>
            </div>
          </div>
          <div className="flex-1 sm:hidden" />
          <button className="relative grid place-items-center w-9 h-9 rounded-xl border border-white/[0.08] text-[#A8A8B0] hover:text-white hover:bg-white/[0.06] transition-all" aria-label="Notificações">
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-white/[0.08]">
            <div className="w-8 h-8 rounded-full bg-[#EA1D2C]/20 text-[#EA1D2C] grid place-items-center text-sm font-bold">
              {user?.email?.[0]?.toUpperCase() ?? 'A'}
            </div>
            <div className="hidden md:block leading-tight">
              <p className="text-sm font-medium text-white truncate max-w-[160px]">{user?.email}</p>
              <p className="text-[11px] text-[#6F6F78]">Administrador</p>
            </div>
            <button onClick={signOut} className="ml-1 text-sm text-[#A8A8B0] hover:text-[#EA1D2C] transition-colors">Sair</button>
          </div>
        </header>
