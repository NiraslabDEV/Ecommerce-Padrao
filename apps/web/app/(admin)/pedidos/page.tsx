  ...
  return (
    <div className="space-y-5">
      ...
      {printer && !printerOnline && (
        <div className="flex items-center justify-between gap-4 rounded-2xl bg-[#EA1D2C]/[0.06] backdrop-blur-[12px] border border-[#EA1D2C]/25 shadow-[0_0_24px_rgba(234,29,44,0.08)] px-5 py-4">
          <div className="flex items-center gap-3">
            <span className="w-2.5 h-2.5 rounded-full bg-[#EA1D2C] animate-pulse" />
            <div>
              <p className="font-semibold text-[#EA1D2C]">Impressora Offline</p>
              <p className="text-sm text-[#A8A8B0]">Verifique a conexão da impressora (print-bridge).</p>
            </div>
          </div>
          <button onClick={fetchDeviceStatus} className="shrink-0 rounded-xl border border-[#EA1D2C]/30 bg-[#EA1D2C]/15 px-4 py-2 text-sm font-semibold text-[#EA1D2C] hover:bg-[#EA1D2C]/25 transition-all">Reconectar</button>
        </div>
      )}
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          <KpiCard label="Total faturado (hoje)" value={hideFaturado ? '••••' : money(stats.faturado_hoje)} accentBorder="border-t-[#22C55E]/60"
            ...
            action={<button onClick={toggleHideFaturado} className="text-[#6F6F78] hover:text-[#EA1D2C] transition-colors" title="Ocultar/mostrar">{hideFaturado ? '🙈' : '👁'}</button>} />
          ...
          <KpiCard label="Avaliação média" value={stats.avg_rating != null ? `${stats.avg_rating} ⭐` : '—'} accentBorder="border-t-amber-400/60"
            delta={dRating != null
              ? <span className={`text-xs font-medium ${dRating > 0 ? 'text-[#22C55E]' : dRating < 0 ? 'text-[#EF4444]' : 'text-[#6F6F78]'}`}>{dRating > 0 ? '▲' : dRating < 0 ? '▼' : '•'} {Math.abs(dRating).toFixed(1).replace('.', ',')} vs ontem</span>
              : <span className="text-xs font-medium text-[#6F6F78]">feedback dos clientes</span>} />
        </div>
      )}
      ...
      <div className={`${GLASS} p-3 flex flex-wrap items-center gap-3`}>
        <div className="flex-1 min-w-[200px] flex items-center gap-2 bg-black/20 border border-white/[0.08] rounded-xl px-3 py-2.5">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6F6F78" strokeWidth="2"><circle cx="11" cy="11" r="7" /><path d="m21 21-4-4" /></svg>
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar pedido, cliente, telefone…"
            className="bg-transparent text-sm text-white placeholder-[#6F6F78] focus:outline-none w-full" />
        </div>
        <div className="flex items-center gap-2 bg-black/20 border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-[#A8A8B0]">
        ...
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}
          className="bg-black/20 border border-white/[0.08] rounded-xl px-3 py-2.5 text-sm text-[#A8A8B0] focus:outline-none focus:border-[#EA1D2C]/40 cursor-pointer">
          {TABS.map((t) => <option key={t.key} value={t.key} className="bg-[#161618] text-white">{t.key === 'all' ? 'Todos os status' : t.label}</option>)}
        </select>
      </div>
          ...
          return (
            <button key={t.key} onClick={() => setStatusFilter(t.key)}
              className={`px-3 py-1.5 rounded-xl text-sm transition-all ${
                active ? 'bg-[#EA1D2C]/15 border border-[#EA1D2C]/30 text-[#EA1D2C] font-semibold' : 'text-[#A8A8B0] hover:bg-white/[0.06] hover:text-white'
              }`}>
              {t.label}
              {c != null && (
                <span className={`ml-1.5 text-[11px] rounded-full px-1.5 ${active ? 'bg-[#EA1D2C] text-white' : 'bg-white/[0.08] text-[#6F6F78]'}`}>{c}</span>
              )}
            </button>
          );
        })}
      ...
      <div className={`hidden md:block ${GLASS} overflow-hidden`}>
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white/[0.03] border-b border-white/[0.06]">
              <tr className="text-left text-[11px] uppercase tracking-wide text-[#6F6F78] font-medium">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} className="px-4 py-10 text-center text-[#A8A8B0]">A carregar pedidos…</td></tr>
              ) : pageOrders.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-10 text-center text-[#A8A8B0]">Nenhum pedido encontrado</td></tr>
              ) : (
                pageOrders.map((order) => {
                  const expanded = expandedId === order.id;
                  return (
                    <Fragment key={order.id}>
                      <tr className={`border-b border-white/[0.04] transition-colors ${expanded ? 'bg-[#EA1D2C]/[0.03]' : 'hover:bg-white/[0.04]'}`}>
                        <td className="px-4 py-3"><div className="font-mono font-bold text-white">{order.order_number}</div><div className="text-[11px] text-[#6F6F78]">{order.flow === 'digital' ? 'Pago online' : 'Manual'}</div></td>
                        <td className="px-4 py-3"><div className="font-medium text-white">{order.customer_name}</div><div className="text-[11px] text-[#A8A8B0]">{order.customer_phone}</div></td>
                        <td className="px-4 py-3"><TypeBadge type={order.fulfillment_type} />{order.payment_proof_path && <div className="text-[11px] text-[#EA1D2C] mt-1">📎 comprovativo</div>}</td>
                        <td className="px-4 py-3 text-sm text-[#A8A8B0] max-w-[220px] truncate">{order.fulfillment_type === 'delivery' ? (order.address || '—') : 'Levantamento no balcão'}</td>
                        <td className="px-4 py-3">
                          <div className="text-sm text-[#e8e8ea]">{formatDate(order.scheduled_for)}</div>
                          <div className="text-[11px] text-[#6F6F78]" title={`Pedido feito: ${format(parseISO(order.created_at), "dd/MM 'às' HH:mm", { locale: pt })}`}>{relativeTime(order.created_at)}</div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button onClick={() => toggleExpand(order)} className="px-3 py-1.5 text-xs font-medium rounded-xl border border-white/[0.08] text-[#A8A8B0] hover:bg-white/[0.06] hover:text-white transition-all whitespace-nowrap">
        ...
        {!loading && orders.length > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-white/[0.06] text-sm text-[#6F6F78]">
      ...
      <div className="md:hidden space-y-3">
        {loading ? (
          <div className="text-center text-[#A8A8B0] py-8">A carregar pedidos…</div>
        ) : pageOrders.length === 0 ? (
          <div className="text-center text-[#A8A8B0] py-8">Nenhum pedido encontrado</div>
        ) : (
          <>
            {pageOrders.map((order) => (
              <div key={order.id} className={`${GLASS} p-4`}>
                <div className="flex items-start justify-between mb-2">
                  <div>
                    ...
                    <div className="text-[11px] text-[#6F6F78]">{order.customer_phone}</div>
                  </div>
                  <StatusPill status={order.status} />
                </div>
                <div className="flex items-center justify-between text-sm mb-3">
                  <span className="text-[#A8A8B0] flex items-center gap-1.5 flex-wrap"><TypeBadge type={order.fulfillment_type} /> · {formatDate(order.scheduled_for)} <span className="text-[11px] text-[#6F6F78]">· {relativeTime(order.created_at)}</span></span>
                  <span className="font-bold text-white">{money(order.total_cents)}</span>
                </div>
                <ActionButtons order={order} />
                <button onClick={() => toggleExpand(order)} className="mt-3 text-xs text-[#EA1D2C] font-medium">
            ...
            {orders.length > PAGE_SIZE && (
              <div className="flex items-center justify-between text-sm text-[#6F6F78] pt-1">
      ...
      {showDenyModal && selectedOrder && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-[4px] flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-white/[0.10] bg-white/[0.06] backdrop-blur-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.6),0_0_40px_rgba(234,29,44,0.08)] p-6 space-y-4">
            ...
            <div className="space-y-1 text-sm text-[#A8A8B0]">
            ...
            <textarea value={denyReason} onChange={(e) => setDenyReason(e.target.value)} rows={4} placeholder="Motivo da recusa…"
              className="w-full rounded-xl bg-black/30 border border-white/[0.08] text-sm text-white placeholder-[#6F6F78] p-3 focus:outline-none focus:border-[#EA1D2C]/50 transition-colors" />
            <div className="flex gap-3">
              <button onClick={() => { setShowDenyModal(false); setSelectedOrder(null); setDenyReason(''); }} className="flex-1 px-4 py-2.5 bg-white/[0.06] hover:bg-white/[0.10] text-[#A8A8B0] rounded-xl transition-all">Voltar</button>
              <button onClick={handleDeny} disabled={!denyReason.trim()} className="flex-1 px-4 py-2.5 bg-[#EA1D2C] hover:bg-[#c8161f] text-white font-semibold rounded-xl transition-all disabled:opacity-50">Confirmar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {message && (
        <div className={`fixed bottom-6 right-6 z-50 rounded-2xl border backdrop-blur-[16px] px-5 py-3 font-medium text-sm shadow-[0_4px_24px_rgba(0,0,0,0.4)] ${
          message.type === 'success' ? 'border-[#22C55E]/30 bg-[#22C55E]/10 text-[#22C55E]' : 'border-[#EA1D2C]/30 bg-[#EA1D2C]/10 text-[#EA1D2C]'
        }`}>
          <div className="flex items-center gap-3">
...
function Delta({ pct, goodUp }: { pct: number | null; goodUp: boolean }) {
  if (pct == null) return <span className="text-xs font-medium text-[#6F6F78]">sem dados de ontem</span>;
  const up = pct >= 0;
  const isGood = pct === 0 ? null : up === goodUp;
  const color = isGood == null ? 'text-[#6F6F78]' : isGood ? 'text-[#22C55E]' : 'text-[#EF4444]';
  ...
  return (
    <div className={`${GLASS} border-t-2 ${accentBorder} p-4 flex flex-col gap-1 hover:bg-white/[0.07] hover:border-white/[0.12] transition-all`}>
      <div className="flex items-center justify-between">
        <span className="text-[11px] uppercase tracking-wide text-[#6F6F78] font-medium">{label}</span>
        {action}
      </div>
...
function Pagination({ page, totalPages, onChange }: { page: number; totalPages: number; onChange: (p: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <button disabled={page <= 1} onClick={() => onChange(page - 1)} className={`${btn} border-white/[0.08] text-[#A8A8B0] hover:bg-white/[0.06] disabled:opacity-40`}>‹</button>
      {pages.map((p, i) => (
        <Fragment key={p}>
          {i > 0 && p - pages[i - 1] > 1 && <span className="text-[#6F6F78] px-0.5">…</span>}
          <button onClick={() => onChange(p)} className={`${btn} ${p === page ? 'bg-[#EA1D2C] text-white border-transparent' : 'border-white/[0.08] text-[#A8A8B0] hover:bg-white/[0.06]'}`}>{p}</button>
        </Fragment>
      ))}
      <button disabled={page >= totalPages} onClick={() => onChange(page + 1)} className={`${btn} border-white/[0.08] text-[#A8A8B0] hover:bg-white/[0.06] disabled:opacity-40`}>›</button>
    </div>
  );
}
...
function OrderDetail({ order, proofUrl, actions }: { order: Order; proofUrl: string | undefined; actions: React.ReactNode }) {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      {/* Itens */}
      <div>
        <div className="text-[11px] font-medium text-[#6F6F78] uppercase tracking-wide mb-2">Itens</div>
        <div className="space-y-1 text-sm">
          {order.items.map((item) => (
            <div key={item.id} className="flex justify-between text-white">
              <span>{item.qty}× {item.name}{item.notes && <span className="text-[#6F6F78] italic"> — {item.notes}</span>}</span>
              <span>{money(item.unit_price_cents * item.qty)}</span>
            </div>
          ))}
        </div>
        {order.notes && <p className="mt-3 text-sm text-[#A8A8B0]"><span className="text-white font-medium">Obs: </span>{order.notes}</p>}
      </div>

      {/* Entrega / Levantamento */}
      <div className="text-sm text-[#A8A8B0] space-y-1">
        <div className="text-[11px] font-medium text-[#6F6F78] uppercase tracking-wide mb-2">{order.fulfillment_type === 'delivery' ? 'Entrega' : 'Levantamento'}</div>
      ...
      <div className="space-y-3">
        <div className="text-sm space-y-1">
          <div className="flex justify-between text-[#A8A8B0]"><span>Subtotal</span><span>{money(order.subtotal_cents)}</span></div>
          {order.delivery_fee_cents > 0 && <div className="flex justify-between text-[#A8A8B0]"><span>Taxa de entrega</span><span>{money(order.delivery_fee_cents)}</span></div>}
        ...
        {order.payment_proof_path ? (
          proofUrl ? (
            <a href={proofUrl} target="_blank" rel="noopener noreferrer" className="block">
              ...
              <span className="text-xs text-[#EA1D2C] hover:underline mt-1 inline-block">Ver comprovativo em tamanho real ↗</span>
            </a>
          ) : <p className="text-sm text-[#A8A8B0]">A carregar comprovativo…</p>
        ) : <p className="text-sm text-[#6F6F78]">Sem comprovativo (pagamento na entrega ou automático).</p>}
