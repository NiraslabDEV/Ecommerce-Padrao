...
export function PromoSection() {
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        <label className="block text-xs font-semibold mb-2" style={{ color: '#A8A8B0' }}>Imagem do cupom</label>
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="text-[11px]" style={{ color: '#6b7280' }}>JPG/PNG/WebP, máx 5 MB</p>
          </div>
        </div>
        <input
          ref={fileRef}
      ...
      {/* Código a exibir */}
      <div className="mb-5">
        <label className="block text-xs font-semibold mb-1.5" style={{ color: '#A8A8B0' }}>Código a mostrar (ex: PRIMEIRABOX)</label>
        <input
          value={promoCode}
          onChange={(e) => setPromoCode(e.target.value.toUpperCase())}
          placeholder="Deixa vazio para não mostrar código"
          maxLength={50}
          className="w-full bg-black/20 border border-white/[0.08] rounded-lg px-3 py-2 text-sm text-white font-mono focus:outline-none focus:border-[#EA1D2C]"
        />
