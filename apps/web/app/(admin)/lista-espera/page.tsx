  ...
  if (loading && page === 0) {
    return (
      <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-[12px] shadow-[0_4px_24px_rgba(0,0,0,0.4)] p-10 text-center">
        <div className="text-[#EA1D2C]">A carregar lista de espera...</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="text-[#A8A8B0] text-sm">
      ...
      {entries.length === 0 ? (
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] backdrop-blur-[12px] shadow-[0_4px_24px_rgba(0,0,0,0.4)] p-10 text-center">
          <div className="text-[#A8A8B0]">Lista de espera vazia</div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {entries.map((entry) => (
            ...
            >
              {/* Nome */}
              <div>
                <div className="text-[#A8A8B0] text-sm mb-1">Nome</div>
                <div className="text-[#e8e8ea] font-semibold">{entry.name}</div>
              </div>

              {/* Telefone */}
              <div>
                <div className="text-[#A8A8B0] text-sm mb-1">Telefone</div>
                <div className="text-[#EA1D2C]">{formatPhone(entry.phone)}</div>
              </div>

              {/* Notas */}
              {entry.notes && (
                <div className="pt-3 border-t border-white/[0.04]">
                  <div className="text-[#A8A8B0] text-sm mb-1">Notas</div>
                  <p className="text-[#e8e8ea] text-sm">{entry.notes}</p>
                </div>
              )}

              {/* Data de registro */}
              <div className="pt-3 border-t border-white/[0.04] text-sm">
                <div className="text-[#A8A8B0]">
                  Registado em{' '}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <button
            ...
            disabled={page === 0}
            className="px-4 py-2 bg-white/[0.08] rounded text-[#e8e8ea] disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/[0.12] transition-colors"
          >
            Anterior
          </button>
