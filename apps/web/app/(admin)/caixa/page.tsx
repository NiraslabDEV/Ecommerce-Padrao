  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-[#EA1D2C] font-semibold animate-pulse">A carregar caixa…</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {d.has_open_session && d.open_session ? (
            <p className="text-sm text-[#A8A8B0] mt-1">
              Sessão aberta desde {fmtTime(d.open_session.opened_at)} ·{' '}
              <span className="text-[#EA1D2C]">período activo</span>
            </p>
          ) : (
            <p className="text-sm text-[#A8A8B0] mt-1">
        ...
        <div className="flex gap-3">
          {!d.has_open_session && (
            <button
              ...
              disabled={submitting}
              className="px-4 py-2 text-sm font-semibold rounded-xl border border-white/[0.08] bg-white/[0.04] text-[#e8e8ea] hover:bg-white/[0.08] transition-all disabled:opacity-50"
            >
              Abrir Sessão
            </button>
          )}
          <button
            ...
            className="px-5 py-2 text-sm font-bold bg-[#EA1D2C] text-white rounded-xl hover:bg-[#c8161f] transition-all shadow-[0_0_16px_rgba(234,29,44,0.25)]"
          >
            Fechar Caixa
          </button>
        </div>
      ...
      <div className={`${GLASS} p-6`}>
        ...
        {d.items_hoje.length === 0 ? (
          <p className="text-[#A8A8B0] text-sm">Nenhum item vendido no período.</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-[11px] uppercase tracking-wide text-[#6F6F78] border-b border-white/[0.06]">
            ...
            <tbody>
              {d.items_hoje.map((item, i) => (
                <tr key={i} className="border-b border-white/[0.04] last:border-0">
                  ...
                  <td className="py-2 text-center text-[#A8A8B0]">{item.qty}</td>
      ...
      <div className={`${GLASS} p-6`}>
        ...
        {d.historico.length === 0 ? (
          <p className="text-[#A8A8B0] text-sm">Nenhum fecho registado ainda.</p>
        ) : (
      ...
      {showCloseModal && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-[4px] flex items-center justify-center z-50 p-4">
          <div className="w-full max-w-md rounded-2xl border border-white/[0.10] bg-white/[0.06] backdrop-blur-[20px] shadow-[0_20px_60px_rgba(0,0,0,0.6),0_0_40px_rgba(234,29,44,0.08)] p-6 space-y-5">
            ...
            <div>
              <label className="block text-sm text-[#A8A8B0] mb-1">Valor Contado (MT)</label>
              <input
                type="number"
                step="0.01"
                min="0"
                ...
                placeholder="0.00"
                className="w-full px-3 py-2 bg-black/30 border border-white/[0.08] rounded-xl text-white focus:outline-none focus:border-[#EA1D2C]/50 text-lg font-mono transition-colors"
              />
            </div>

            {countedInput && (
              <div className="bg-black/20 border border-white/[0.08] rounded-xl p-3 text-sm flex justify-between">
                <span className="text-[#A8A8B0]">Diferença</span>
            ...
            <div>
              <label className="block text-sm text-[#A8A8B0] mb-1">Notas (opcional)</label>
              <textarea
                value={notes}
                onChange={(e) => setNotes(e.target.value)}
                rows={2}
                placeholder="Observações sobre o fecho…"
                className="w-full px-3 py-2 bg-black/30 border border-white/[0.08] rounded-xl text-white focus:outline-none focus:border-[#EA1D2C]/50 resize-none text-sm transition-colors"
              />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                disabled={submitting}
                className="flex-1 px-4 py-2 text-sm text-[#A8A8B0] bg-white/[0.06] hover:bg-white/[0.10] rounded-xl transition-all"
              >
                Cancelar
              </button>
              <button
                ...
                className="flex-1 px-4 py-2 text-sm font-bold bg-[#EA1D2C] text-white rounded-xl hover:bg-[#c8161f] transition-all disabled:opacity-50"
              >
...
function StatCard({ label, value, highlight, accentBorder }: { label: string; value: string; highlight?: boolean; accentBorder: string }) {
  return (
    <div className={`${GLASS} border-t-2 ${accentBorder} p-5 hover:bg-white/[0.07] hover:border-white/[0.12] transition-all`}>
      <p className="text-[11px] text-[#6F6F78] uppercase tracking-wide mb-2 font-medium">{label}</p>
      <p className={`text-2xl font-bold ${highlight ? 'text-[#EA1D2C]' : 'text-white'}`}>{value}</p>
    </div>
  );
}
...
function Row({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className="flex justify-between">
      <span className="text-[#A8A8B0]">{label}</span>
      <span className={bold ? 'text-white font-bold' : 'text-[#e8e8ea]'}>{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-4 p-3 bg-black/20 border border-white/[0.08] rounded-xl">
      <div className="flex-1 min-w-0">
        <p className="text-xs text-[#A8A8B0]">{fmtDT(item.opened_at)} → {fmtDT(item.closed_at)}</p>
        {item.notes && <p className="text-xs text-[#6F6F78] mt-0.5 truncate">{item.notes}</p>}
      </div>
      ...
      <div className="flex gap-2 shrink-0">
        <button
          ...
          className="px-3 py-1.5 text-xs rounded-xl border border-white/[0.08] bg-white/[0.04] text-[#A8A8B0] hover:text-white hover:bg-white/[0.08] transition-all"
        >
          PDF
        </button>
        <button
