...
export function PaysuiteSection() {
  ...
  if (loading) return <p className="text-sm text-[#A8A8B0]">A carregar configuração…</p>;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h3 className="text-lg font-bold text-[#EA1D2C]">Pagamento Automático (Paysuite)</h3>
      ...
      <div className="space-y-1">
        <label className="block text-sm font-medium text-[#e8e8ea]">Método de pagamento</label>
        <select
          ...
          className="w-full rounded-lg bg-black/30 border border-white/[0.08] px-3 py-2 text-sm text-[#e8e8ea] focus:border-[#EA1D2C] focus:outline-none"
        >
        ...
        {SECRET_FIELDS.map(({ key, label, help }) => {
          ...
          return (
            <div key={key} className="space-y-1">
              <label className="block text-sm font-medium text-[#e8e8ea]">{label}</label>
              {isEditing ? (
                <input
                  type="password"
                ...
                <div className="flex items-center gap-3">
                  <span className="text-sm text-[#A8A8B0] font-mono">
                    {stored ? maskToken(stored) : 'não definido'}
                  </span>
                  <button
                    type="button"
                    onClick={() => setEditingSecret((s) => ({ ...s, [key]: true }))}
                    className="text-xs text-[#EA1D2C] hover:underline"
                  >
                    {stored ? 'Substituir' : 'Definir'}
                  </button>
                </div>
              )}
              <span className="block text-xs text-[#A8A8B0]">onde encontrar: {help}</span>
            </div>
          );
        })}
      </section>
      ...
      <div className="space-y-1">
        <label className="block text-sm font-medium text-[#e8e8ea]">
          Webhook URL (colar no painel Paysuite)
        </label>
        <code className="block rounded-lg bg-black/30 border border-white/[0.08] px-3 py-2 text-xs text-[#EA1D2C] break-all">
          {webhookUrl || '…'}
        </code>
        <p className="text-xs text-[#A8A8B0]">
      ...
      <button
        ...
        onClick={save}
        disabled={saving}
        className="rounded-lg bg-[#EA1D2C] px-6 py-2.5 text-sm font-semibold text-white hover:bg-[#c8161f] disabled:opacity-60"
      >
