...
type MenuItemLite = { id: string; name: string; price_cents: number; variants?: MenuVariant[]; addons?: MenuAddon[] };

// preço unitário = (variante ou base) + Σ adicionais. PREVIEW; servidor é a verdade.
function lineUnitPrice(item: MenuItemLite | undefined, variantId?: string, addonIds: string[] = []): number {
  if (!item) return 0;
...
function Opt({ selected, onClick, children, className = '' }: { selected: boolean; onClick: () => void; children: React.ReactNode; className?: string }) {
  return (
    ...
    >
      {children}
      {selected && (
        <span className="absolute top-1.5 right-1.5 w-5 h-5 rounded-full grid place-items-center text-white text-[11px] font-bold shadow" style={{ background: 'var(--st-grad)' }}>✓</span>
      )}
    </button>
  );
}
...
export default function CheckoutPage() {
  ...
  const subtotal = cart.reduce((sum, item) => {
    ...
    return sum + lineUnitPrice(menuItem, item.variantId, item.addonIds) * item.qty;
  }, 0);
  ...
  function cartTrackItems(): TrackItem[] {
    ...
    return cart
      .map((line) => {
        ...
        return it
          ? { id: it.id, name: it.name, price_cents: lineUnitPrice(it, line.variantId, line.addonIds), qty: line.qty }
          : null;
      })
        ...
        <div className="max-w-[480px] mx-auto md:mx-0 md:w-full md:max-w-lg">
          <div className={CARD + ' p-6'}>
            ...
            <button
              ...
              className="w-full text-white font-bold py-4 px-4 rounded-2xl transition-all active:scale-[0.99] disabled:opacity-50"
      ...
      <div className="max-w-[480px] md:max-w-4xl lg:max-w-5xl mx-auto pb-8">
          ...
          <div className={CARD}>
            ...
            {hasAutoPayment && (
              <div className="flex bg-[var(--st-bg)] rounded-lg p-1 mb-4 border border-[var(--st-line)]">
                <button onClick={() => setPaymentFlow('manual')} className={`flex-1 py-2 rounded-md text-sm font-semibold transition-all ${paymentFlow === 'manual' ? 'text-white' : 'text-[var(--st-muted)]'}`} style={paymentFlow === 'manual' ? { background: 'var(--st-grad)' } : undefined}>
                  Comprovativo
                </button>
                <button onClick={() => setPaymentFlow('auto')} className={`flex-1 py-2 rounded-md text-sm font-semibold transition-all ${paymentFlow === 'auto' ? 'text-white' : 'text-[var(--st-muted)]'}`} style={paymentFlow === 'auto' ? { background: 'var(--st-grad)' } : undefined}>
                  Pagar Agora
                </button>
              </div>
            )}
            ...
            {referralCode && couponResult?.valid ? (
              ...
              <div className="rounded-xl px-4 py-3 flex items-start gap-3" style={{ background: '#0f2a1a', border: '1px solid #22c55e55' }}>
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-mono font-extrabold text-sm text-white">{referralCode}</span>
              ...
              <>
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="px-5 rounded-xl font-bold text-white text-sm shrink-0 disabled:opacity-50"
                    style={{ background: 'var(--st-grad)' }}
                  >
        ...
        <div className="space-y-4 mt-4 md:mt-0 md:sticky md:top-20">
          ...
          <button
            ...
            className="w-full text-white font-extrabold py-4 px-4 rounded-2xl transition-all active:scale-[0.99] disabled:opacity-50"
            style={{ background: 'var(--st-grad)' }}
          >
            {isSubmitting
