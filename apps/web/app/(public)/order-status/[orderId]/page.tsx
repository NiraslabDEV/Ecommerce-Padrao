      ...
      <div className="max-w-[480px] mx-auto pb-8">
        ...
        <header className="sticky top-0 z-10 bg-[var(--st-bg)]/95 backdrop-blur border-b border-[var(--st-line)]">
          <div className="px-4 py-4 flex items-center gap-3">
            ...
            <h1 className="text-xl font-extrabold text-white">Status do Pedido</h1>
          </div>
        </header>
        ...
        <div className="px-4 mt-4 space-y-4">
          ...
          <div className={CARD}>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-white font-extrabold text-xl">{orderStatus.order_number}</p>
              </div>
          ...
          {/* Detalhes */}
          <div className={CARD}>
            <h2 className="text-white font-bold mb-4">Detalhes</h2>
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between pt-2 border-t border-[var(--st-line)]">
                ...
                <span className="text-white font-extrabold text-base">{formatMT(orderStatus.total_cents as Cents)}</span>
              </div>
            </div>
          </div>
          ...
          {isTerminal && (
            <>
              {orderStatus.status === 'delivered' && !orderStatus.feedback_submitted && (
                <div className={CARD}>
                  <h2 className="text-white font-bold mb-3">Como foi a sua experiência?</h2>
          ...
          <div key={i} className="flex-1 flex flex-col items-center relative">
            {i > 0 && (
              <div
                ...
                style={{ background: i <= active ? 'var(--st-primary)' : 'rgba(255,255,255,0.12)' }}
              />
            )}
            <div
              className="w-[22px] h-[22px] rounded-full grid place-items-center text-[10px] font-extrabold text-white relative z-[1] mb-1.5"
              style={{ background: reached ? 'var(--st-primary)' : 'transparent', border: `2px solid ${reached ? 'var(--st-primary)' : 'rgba(255,255,255,0.2)'}` }}
            >
              {done ? '✓' : isActive ? '•' : ''}
            </div>
            <span
              className="text-[9.5px] text-center leading-tight"
              style={{ color: isActive ? '#fff' : done ? 'var(--st-muted)' : 'rgba(255,255,255,0.3)', fontWeight: isActive ? 800 : 400 }}
            >
              {label}
            </span>
          </div>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="flex justify-center gap-2">
        {[1, 2, 3, 4, 5].map((star) => (
          <button key={star} type="button" onClick={() => setRating(star)} className="text-3xl transition-colors" style={{ color: star <= rating ? 'var(--st-star)' : 'rgba(255,255,255,0.25)' }}>
            {star <= rating ? '★' : '☆'}
          </button>
        ))}
      </div>
      <textarea
        ...
        className="w-full rounded-xl p-3 text-white placeholder:text-[var(--st-muted)] focus:outline-none focus:border-[var(--st-primary)] border border-[var(--st-line)] bg-[var(--st-bg)]"
        rows={3}
      />
