...
export default function PaymentReturnPage() {
  ...
  if (pollStatus === 'failed') {
    return (
      <Wrap>
        ...
        <h1 className="text-2xl font-bold text-white mb-2">Pagamento Não Processado</h1>
        <p className="text-[var(--st-muted)] mb-6">O pagamento não foi concluído. Pode tentar novamente.</p>
        <button onClick={() => { localStorage.removeItem('pending_order_id'); router.push('/checkout'); }} className="w-full text-white font-bold py-4 px-4 rounded-2xl" style={{ background: 'var(--st-grad)' }}>
          Tentar Novamente
        </button>
  ...
  if (pollStatus === 'timeout') {
    return (
      <Wrap>
        ...
        <h1 className="text-xl font-bold text-white mb-2">A verificar pagamento…</h1>
  ...
  return (
    <Wrap>
      ...
      <h1 className="text-2xl font-bold text-white mb-2">A processar pagamento</h1>
