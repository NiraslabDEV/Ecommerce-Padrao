...
beforeAll(async () => {
  ...
  const { data: gift } = await admin
    .from('menu_items')
    .upsert({ name: 'Brinde Teste', price_cents: 0, category_id: cat.id, available: true, is_gift: true, sort: 999 }, { onConflict: 'name' })
    .select('id')
    .single();
  if (!gift) throw new Error('Falha ao criar item brinde');
  giftMenuItemId = gift.id;

  // Código de brinde (free_item)
  const { data: rc } = await admin
    ...
    .select('id')
    .single();
  if (!rc) throw new Error('Falha ao criar código de brinde');
  codeId = rc.id;

  // Código de desconto em centavos
  const { data: dc } = await admin
    ...
    .select('id')
    .single();
  if (!dc) throw new Error('Falha ao criar código de desconto');
