...
describe('(c) Rollback total quando item esgota', () => {
  it('faz rollback total quando stock é insuficiente durante confirm_payment', async () => {
    ...
    const { data: result, error } = await staff.rpc('confirm_payment', {
      ...
      p_order_id:        orderId,
      p_provider:        'mock',
      p_provider_ref:    'mock_ref_stock_004',
      p_method:          'mpesa',
    ...
    const { data: item } = await admin
      ...
      .eq('id', menuItemId)
      .single();
    expect(item!.stock_qty).toBe(2); // Still 2, rollback successful

    // Order should still be awaiting_payment
    ...
    const { data: events } = await admin
      ...
      .select('type')
      .eq('order_id', orderId)
      .eq('type', 'payment.stock_insufficient');
